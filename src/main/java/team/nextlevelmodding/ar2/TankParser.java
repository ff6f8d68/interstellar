...
import team.nextlevelmodding.ar2.blocks.BipropellantTank;
import team.nextlevelmodding.ar2.blocks.Bottomtank;
import team.nextlevelmodding.ar2.blocks.Middletank;
import team.nextlevelmodding.ar2.blocks.Toptank;
...
public class TankParser {
    ...
    private static void explore(Level world, BlockPos pos, Set<BlockPos> visited) {
        ...
        if (!isConnectable(block) && !(block instanceof Toptank)
            && !(block instanceof Middletank)
            && !(block instanceof Bottomtank)
            && !(block instanceof BipropellantTank)) {
            return;
        }
